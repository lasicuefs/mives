...
/* Função para ocutar linhas não usadas*/
function ocutarLinasNaoUsadas() {
	var pai = document.getElementById("versos");
	for (var i = 0; i < pai.children.length; i++) {
		var id = pai.children[i].children[0].id;
		var text = document.getElementById(id).textContent;
		if (text === "") {
			document.getElementById(pai.children[i].id).style.display = 'none';
		}

	}

} function redefinirLinhasNaoUsadas() {
	var pai = document.getElementById("versos");
	for (var i = 0; i < pai.children.length; i++) {
		var id = pai.children[i].children[0].id;
		document.getElementById(pai.children[i].id).style.display = 'block';
	}
